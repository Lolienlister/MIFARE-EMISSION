...
T3   probe_counter ← (stored ? stored.counter + 1 : 0)   ── ожидаемое значение на карте
...
T8   AccessEngine.decide(uid, counter, now):
       ...
       e. иначе                               → ALLOW; на карту пишется counter+1,
                                                в state — counter (катап до последнего
                                                видимого значения, см. §4 «инварианты»).
T9   if ALLOW:
...
| Tools               | `tools/emission_tool.cpp` | `gen-master`, `emit`, `reset`, `revoke` CLI |
...
function handle_transaction(now):
    ...
    top       := stored ? stored.counter + 1 : 0        // ожидаемое card.counter после прошлого ALLOW
    ...
    // state хранит «последнее видимое» card.counter, не значение, записанное
    // только что на карту (см. §4.3 «инвариант счётчика»).
    state.upsert(uid, {counter=counter, status=OK, last_seen=now})
    return ALLOW
```
...
function decide(uid, counter_card, now):
    ...
    if stored:
        ...
        // первое появление UID — создаём запись с counter_card
        if counter_card >= MAX_COUNTER - 1:      return DENY(COUNTER_OVERFLOW)
    state.upsert(uid, {counter=counter_card, status=OK, last_seen=now})
    return ALLOW         // вызывающий код пишет на карту counter_card + 1
```

**Инвариант счётчика.** После успешной транзакции выполняется:

```
card.counter        = counter_card + 1
card.sector_key     = KDF(MASTER_KEY, uid, card.counter)   // = KDF(uid, counter_card + 1)
state.counter[uid]  = counter_card                          // «последнее видимое card.counter»
```

Поэтому следующая корректная транзакция читает `card.counter > stored.counter`
и попадает в ветку ALLOW. Если бы `state.counter` обновлялось до того же
значения, что и `card.counter` после записи, то любая следующая легитимная
транзакция выглядела бы как `REPLAY`. Эмиссия (`emit`) учитывает этот
инвариант и сразу выставляет `card.counter = 1`, `state.counter = 0`.
